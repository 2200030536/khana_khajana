...
const MessUserSignup = () => {
  ...
  const [formData, setFormData] = useState({
    name: '',
    id: '',  // Changed from messName to id to match schema
    email: '',
    password: '',
    confirmPassword: ''
  });
  ...
  const validateForm = () => {
    ...
    } else if (isNaN(formData.id) || !Number.isInteger(Number(formData.id))) {
      newErrors.id = 'User ID must be a whole number';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        name: formData.name,
        id: parseInt(formData.id), // Convert to number to match schema
        email: formData.email,
        password: formData.password,
        userType: 'messUser' // Important to set the user type
      };
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <motion.div variants={itemVariants}>
                    <TextField
                      ...
                      onChange={handleChange}
                      error={!!errors.id}
                      helperText={errors.id || "Please enter a unique numeric ID"}
                      InputProps={{
                        startAdornment: (
                          <InputAdornment position="start">
                            <NumbersIcon sx={{ color: '#F57C00' }} />
                          </InputAdornment>
                        ),
                      }}
                      required
                ...
                <Grid item xs={12}>
                  <motion.div variants={itemVariants}>
                    ...
                    >
                      {loading ? <CircularProgress size={24} color="inherit" /> : 'Register Mess Account'}
                    </Button>
                  </motion.div>
                </Grid>
              </Grid>
